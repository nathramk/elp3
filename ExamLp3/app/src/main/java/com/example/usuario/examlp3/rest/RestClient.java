package com.example.usuario.examlp3.rest;
...
public class RestClient {
	...
	private HttpUriRequest addBodyParams(HttpUriRequest request)
		...
		if (jsonBody != null) {
			...
			if (request instanceof HttpPost)
				((HttpPost) request).setEntity(new StringEntity(jsonBody, "application/json; charset=UTF-8"));
			else if (request instanceof HttpPut)
				((HttpPut) request).setEntity(new StringEntity(jsonBody, "application/json; charset=UTF-8"));
	...
	private void executeRequest(HttpUriRequest request, String url) {
		...
		HttpConnectionParams.setConnectionTimeout(params, 30 * 10000);
		HttpConnectionParams.setSoTimeout(params, 30 * 10000);
